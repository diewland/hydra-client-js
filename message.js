var Message = function(config){
	...
	var _proxy_fn_wait  = true;
	var _proxy_fn_resp  = undefined;
    ...
    socket.on('return', function(msg){
		...
		//
		self.log('return', msg);
		_proxy_fn_wait = false;
		if(msg.value){
			_proxy_fn_resp = JSON.parse(msg.value);
		}
		else {
			self.err(msg.message);
		}
    });
	...
	this.proxy_fn = function(fn_name){
		return function(){
			...
			// TODO ( promise needed )
			result = _proxy_fn_resp;

			// reset proxy_fn vars
			_proxy_fn_resp = undefined;
			_proxy_fn_wait = true;
